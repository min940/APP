# 회전 컨트롤 (Rotation Control)

화면 회전을 손쉽게 제어하는 오프라인 유틸리티입니다. 갤럭시 S24(One UI) 기준으로 설계되었습니다.

주요 기능:
- **화면 회전 위젯**: 홈 화면 위젯을 누르면 전체 화면이 가로↔세로로 토글
- **앱 연동 바로가기**: 바로가기 탭 → 가로 회전 + 지정 앱 실행 → 앱을 벗어나면 자동 세로 복귀
- **앱별 회전**(접근성 방식): 특정 앱 실행 시에만 원하는 방향으로 회전
- **마지막 탭 저장**: 다음 실행 시 마지막으로 보던 화면으로 복원
